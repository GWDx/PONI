...
import semexp.envs.utils.rotation_utils as ru
import torch
...
def get_point_cloud_from_z_t(Y_t, camera_matrix, device, scale=1, grids=None):
    """Projects the depth image Y into a 3D point cloud.
    Inputs:
        Y is ...xHxW
        camera_matrix
    ...
    """
    if grids is None:
        grid_x, grid_z = get_meshgrid(Y_t, device)
    else:
        grid_x, grid_z = grids
