...
class TransferEvaluator(BaseRLTrainer):
    ...
    def _synchronize_config(self):
        ...
        self.config.IMAGE_SEGMENTATION.min_depth = (task_cfg.SIMULATOR.DEPTH_SENSOR.MIN_DEPTH)
        self.config.IMAGE_SEGMENTATION.max_depth = (task_cfg.SIMULATOR.DEPTH_SENSOR.MAX_DEPTH)
        # Update the global agent config
        self.config.GLOBAL_AGENT.env_frame_width = task_cfg.SIMULATOR.DEPTH_SENSOR.WIDTH
        self.config.GLOBAL_AGENT.env_frame_height = (task_cfg.SIMULATOR.DEPTH_SENSOR.HEIGHT)
        self.config.GLOBAL_AGENT.camera_height = (task_cfg.SIMULATOR.DEPTH_SENSOR.POSITION[1])
        # GT segmentation
    ...
    def eval(self) -> None:
        ...
        writer = TensorboardWriter(self.config.TENSORBOARD_DIR, flush_secs=self.flush_secs)
        ...
        if number_of_eval_episodes == -1:
            ...
            if total_num_eps < number_of_eval_episodes:
                logger.warn(f"Config specified {number_of_eval_episodes} eval episodes"
        ...
        while (len(stats_episodes) < number_of_eval_episodes and agent_states["finished"].sum() < self.envs.num_envs
            ...
            wait_mask = ((agent_states["wait_env"] == 1) | (agent_states["finished"] == 1)).tolist()
            ...
            for e, done in enumerate(dones):
                ...
                if done:
                    ...
                    agent_states = self.global_agent.init_map_and_pose_for_env(agent_states, e)

                    pbar.update()
                    episode_stats = {}
                    episode_stats["reward"] = current_episode_reward[e].item()
                    episode_stats["goal_distance"] = current_episodes[e].info["geodesic_distance"]
                ...
                # episode continues
                    ...
                    rgb_image = (batch["rgb"][e].cpu().numpy().astype(np.uint8))  # (H, W, C)
                    ...
                    # Add GT top-down map
                    top_down_map = maps.colorize_draw_agent_and_fit_to_height(infos[e]["top_down_map"], frame.shape[0])
